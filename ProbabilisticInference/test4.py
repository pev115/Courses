...
def Covariance(theData):
  ...
  realData -= mean
  #############################matrixData = matrix(realData)
  
  #print shift_data
  #print "boum",transpose(shift_data)
  
  
  
  covar = dot( transpose(realData), realData)
  
  #############################covar = matrixData.T*matrixData
  
  
 #print "rows", covar.shape[0], "colums", covar.shape[1]
  
  
  covar= covar/(noDataPoints-1)
  
 # print "chou" , theData[1], "couhcouh" ,mean
  #print theData[1]-mean
 # print  "transp", array(( theData[1]-mean)).T
 # print (theData[1]-mean)
  #print dot(transpose(theData[1]-mean),(theData[1]-mean))
  #for i in range(noDataPoints):
  # covar +=  dot((theData[i]-mean), transpose(theData[i]-mean))
...
'''  
  dot((theData- mean), transpose((theData-mean)))

  print covar
  covar = covar/float(noDataPoints-1)
  ...
  for i in range(noVariables):
	...
	covar[j,i] += float((i-mean[i])*(j-mean[j])) / float((noDataPoints-1))
...
def ProjectFace(theBasis, theMean, theFaceImage):
    ...
    centered_image = subtract(theface, mean)
    
    #basis = matrix(theBasis)
   # print "mean shappew" , mean
   # print "centeed shape", centered_image.shape
    #print "basis shape" , theBasis.shape
    ...
    #b=matrix([1,2,3,4])
    #testing =  b.T * a
    #print "testing," ,   testing.shape, testing
    
    
    
    
   
   # magnitudes = dot(centered_image,dot(transpose(theBasis),theBasis))
    
    #print "da big shape yo MMMAAAAAGGGGGGGGG" ,magnitudes.shape, magnitudes
...
def CreatePartialReconstructions(aBasis, aMean, componentMags):
  ...
  print "SHAAAAAPEEEEEEEEEE " , componentMags.shape, componentMags.shape[0], componentMags
  for i in range(componentMags.shape[0]):
   # print "dashape",  aBasis[i].shape
   # print "dashape2", componentMags[i]
...
def PrincipalComponents(theData):
 ...
 # print "SHAPE " ,theData.shape
  mean = Mean(theData)
   
  #print "THEDATTTTAAAAAAA", theData
  #print "chakan", mean, mean.shape
  
  centered_data = theData - mean
  #print "caroumba",  centered_data
  
  transform_matrix = dot(centered_data,transpose(centered_data))
  
  eigenvalues, eigenvectors = linalg.eig(transform_matrix)
  
 # print "EEEEEEEEEIIIIGEEEEENNNNNNNN" ,   eigenvalues, eigenvectors
  #print "EEEEIIIGENENNNNNNN!!!!!!", eigenvectors[:,0]
  
  eigenvectors_final = dot(transpose(centered_data), eigenvectors)
  #print "final eigenvectors" , eigenvectors_final.shape , eigenvectors_final
 # print "eignevalues ", eigenvalues
 
  sort_order = eigenvalues.argsort()[::-1]
  #print "ordeeerrrr",  sort_order
  eigenvalues = eigenvalues[sort_order]
  eigenvectors_final = eigenvectors_final[:,sort_order]
  
  #print "AGAIN final eigenvectors" , eigenvectors_final.shape , eigenvectors_final
  #print "AGAAINNN eignevalues ", eigenvalues
  ...
  #print "final eigenvectors" , eigenvectors_final.shape[0]
  #print "final eigenvectors" , eigenvectors_final.shape[0]
  for i in range(eigenvectors_final.shape[0]):
    eigenvectors_final[i] = eigenvectors_final[i]/linalg.norm(eigenvectors_final[i])
    

  #identity = dot(eigenvectors_final, eigenvectors_final.transpose())
 # print "IDENTITTYYYYYY" , identity  
  # Coursework 4 task 6 ends here
  
 # print eigenvectors_final
  return eigenvectors_final
...
AppendString("IDAPIResults03.txt","Coursework Three results by Pierretranspose(theBasis) Eugene Valassakis (pev115, 00644379)")
AppendString("IDAPIResults03.txt","") #blank line
...
#print dot(theBasis, theBasis.transpose())

#componentMags=  ProjectFace(theBasis,  "MeanImage.jpg","c.pgm")
#CreatePartialReconstructions(theBasis, "MeanImage.jpg", componentMags)
read_values= ReadImages()
theData = array(read_values)
#print "RREADED VALUES", read_values[2]
#print "THE DATA" , theData
#print "SHHHHHHHAAAAAAAPPPEEEE"  ,theData.shape


pca = PrincipalComponents(theData)
CreateEigenfaceFiles(pca)
componentMags=  ProjectFace(pca,  "MeanImage.jpg","c.pgm")
CreatePartialReconstructions(pca, "MeanImage.jpg", componentMags)
